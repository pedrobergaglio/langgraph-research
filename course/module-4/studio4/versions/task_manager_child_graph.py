...
def select_procedure(state: GeneralState):
    ...
    return {"stored_procedures": stored_procedures,
            ...
            "human_feedback_message": "approve", #we return the selected procedure and reset the feedback
            } 
...
def human_creation(state: CreatorState):
    pass
...
def executor(state: GeneralState):
    ...
    for action in actions:
        ...
        if human_confirmation_message == 'yes' and not confirmed:
...
def create_procedure_function(state: CreatorState):
    ...
    return {
        ...
        "messages": [AIMessage(content=procedure_creation.message)],
...
def decide_create_done(state: CreatorState):
    ...
    messages = state.get('human_feedback_message', "")

    if messages == "":
        return "save_procedure"
    
    else: return "create_procedure_function"
...
creator_builder = StateGraph(CreatorState) #initialization of the creator graph
creator_builder.add_node("create_procedure_function", create_procedure_function)
creator_builder.add_node("human_creation", human_creation)
creator_builder.add_node("save_procedure", save_procedure)

creator_builder.add_edge(START, "create_procedure_function")
creator_builder.add_edge("create_procedure_function", "human_creation")
creator_builder.add_conditional_edges("human_creation", decide_create_done, ["create_procedure_function", "save_procedure"])
creator_builder.add_edge("save_procedure", END)
...
builder.add_node("create_procedure", creator_builder.compile(interrupt_before=['human_creation'])) #initialization of the creator graph
builder.add_node("start", start)

# Logic
builder.add_edge(START, "start")
builder.add_conditional_edges("start", decide_create_select, ["create_procedure", "select_procedure"])
builder.add_edge("create_procedure", "start")
...
builder.add_conditional_edges("human_confirmation", decide_execute_end, ["executor", "start"])

# Compile
graph = builder.compile(interrupt_before=['human_feedback', 'start'], interrupt_after=['create_procedure'])