...
search_query_instructions = SystemMessage(content=f"""
...
def search_web(state: InterviewState):
    ...
    search_query = structured_llm.invoke([search_query_instructions]+state['messages'])
...
def write_section(state: InterviewState):
    ...
    # Get state
    # interview = state["interview"] # Optionally, use raw interview text
    context = state["context"]
    analyst = state["analyst"]
...
def initiate_all_interviews(state: ResearchGraphState):
    ...
    if human_analyst_feedback.lower() != 'approved':
...
# Add nodes and edges for outer graph
